...
function getTopFive(callbackForShow) {
  ...
  fetch(`https://api.twitch.tv/kraken/games/top?limit=${limit}`,
    {
      headers: {
    ...
    .then((response) => response.json()) // .json 會 return Promise
    .catch((err) => window.alert(`發生錯誤，請重新整理頁面再試一遍，或與程式管理員聯絡，謝謝您。\n${err}`))
    .then((response) => {
      const result = response.top.map((x) => `${x.game.name}`);
      topFive = result;
...
function getStream(gameName, offset, callbackForShow) {
  ...
  fetch(`https://api.twitch.tv/kraken/streams/?game=${encodeURIComponent(gameName)}&limit=${limit}&offset=${offset}`,
    {
      headers: {
        Accept: 'application/vnd.twitchtv.v5+json',
        'Client-ID': 'n1k78ho4sgcual55b8sdmb40s5gogr',
    ...
    .catch((err) => window.alert(`在進行連線時發生錯誤，請檢查網路連線，謝謝您。\n${err}`))
    .then((response) => {
      if (typeof callbackForShow === 'function') {
        callbackForShow(response, limit);
      } else {
        console.log('親愛的程式員，你沒有傳顯示到畫面的方法，所以只有撈到資料');
      }
